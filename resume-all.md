...
编辑器（Vim），包管理（pip），多版本管理（pyenv），虚拟环境（virtualenv），环境设置（autoenv），编码风格（PEP8），编码风格检查（flake8），单元测试（unittest），异常追踪（traceback），日志（logging），性能分析（cProfile），文档（Sphinx），数据库版本管理（migrate），代码托管（Git），文档格式（MarkDown）
